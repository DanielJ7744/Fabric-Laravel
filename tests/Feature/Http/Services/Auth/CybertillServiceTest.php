 ...
 * @group connectors
 */
class CybertillServiceTest extends LaravelTestCase
{
    use RefreshDatabase;
    ...
    public function setup(): void
    {
        ...
        $this->system = factory(System::class)->create(['factory_name' => 'Cybertill']);
    ...
    public function test_credentials_validation(): void
    {
        ...
        $this
            ...
            ->assertJsonValidationErrors([
                'credentials' => implode(' ', [
                    'The ct number field is required.',
                    'The version field is required.',
                    'The ct auth id field is required.',
                    'The ct soap url field is required.'
                ])
            ]);
    }
    ...
    {
        $credentials = [
            'ct_number' => 'test_number',
            'version' => '1.5',
            'ct_auth_id' => 'test_auth_id',
            'ct_soap_url' => 'www.url.com',
            'connector_name' => 'test',
            'timezone' => 'UTC',
            'date_format' => 'd/m/Y',
        ...
        $mock = $this->partialMock(CybertillService::class, fn ($mock) => $mock
            ->shouldReceive('authenticate')
            ->andReturn(['session_id' => 'test_id']));

        SystemAuth::partialMock()
            ->shouldReceive('driver')
            ->once()
            ->with('Cybertill', $credentials)
            ->andReturn($mock);
