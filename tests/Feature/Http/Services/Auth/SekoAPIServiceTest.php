 ...
 * @group connectors
 */
class SekoAPIServiceTest extends LaravelTestCase
{
    use RefreshDatabase;
    ...
    public function setup(): void
    {
        ...
        $this->system = factory(System::class)->create(['factory_name' => 'SekoAPI']);
    ...
    {
        $credentials = [
            'token' => 'asdfghjklqwertyuiopzxcvbnm123453456789df',
            'url' => 'https://ihubuat.supplystream.com',
            'connector_name' => 'test',
            'timezone' => 'UTC',
            'date_format' => 'd/m/Y',
        ...
        $mock = $this->partialMock(SekoAPIService::class, fn ($mock) => $mock
            ->shouldReceive('authenticate')
            ->andReturn(['CallStatus' => ['Success' => true]]));

        SystemAuth::partialMock()
            ->shouldReceive('driver')
            ->once()
            ->with('SekoAPI', $credentials)
            ->andReturn($mock);
