 ...
 * @group connectors
 */
class MiraklServiceTest extends LaravelTestCase
{
    use RefreshDatabase;
    ...
    public function setup(): void
    {
        ...
        $this->system = factory(System::class)->create(['factory_name' => 'Mirakl']);
    ...
    public function test_authenticate_method(): void
    {
        ...
        $mock = $this->partialMock(MiraklService::class, fn ($mock) => $mock
            ->shouldReceive('authenticate')
            ->andReturn(['shop_id' => 200]));

        SystemAuth::partialMock()
            ->shouldReceive('driver')
            ->once()
            ->with('Mirakl', $credentials)
            ->andReturn($mock);
