...
use App\Http\Services\Webhook\ShopifyService as ShopifyWebhookService;
...
class ShopifyWebhookTest extends LaravelTestCase
{
    use RefreshDatabase;

    protected string $systemFactoryName = 'Shopify';
    ...
    public function setUp(): void
    {
        ...
        $this->authMock = $this->partialMock(ShopifyAuthService::class, fn ($mock) => $mock
        ...
        SystemAuth::partialMock()
            ...
            ->with('Shopify', $this->credentials)
            ->andReturn($this->authMock);

        $webhookMock = $this->partialMock(ShopifyWebhookService::class, fn ($mock) => $mock
            ->shouldReceive('unsubscribe')
            ->andReturn(true));

        SystemWebhook::partialMock()
            ->shouldReceive('driver')
            ->with('Shopify', [], $this->authMock)
    ...
    public function test_can_create_with_permission(): void
    {
        ...
        SystemWebhook::partialMock()
            ...
            ->with(
                'Shopify',
                ['eventType' => $this->eventType->key, 'callbackUrl' => $webhook->getCallbackUrl(), 'fields' => ['id']],
                $this->authMock
            )
            ->andReturn($webhookMock);
