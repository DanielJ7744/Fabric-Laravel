...
class FilterTemplateControllerTest extends LaravelTestCase
{
    use RefreshDatabase;
    ...
    public function test_user_with_permission_can_retrieve_filter_templates(): void
    {
        $this
            ->passportAs($this->withPermissions)
            ->getJson(route('api.v2.filter-templates.index'))
            ->assertOk();
    }

    public function test_user_without_permission_cannot_retrieve_filter_templates(): void
    {
        $this
    ...
    public function test_user_with_permission_can_retrieve_a_filter_template(): void
    {
        $this
            ->passportAs($this->withPermissions)
            ->getJson(route('api.v2.filter-templates.show', $this->filterTemplate))
            ->assertOk()
            ->assertJsonPath('data.id', $this->filterTemplate->id);
    }
}
